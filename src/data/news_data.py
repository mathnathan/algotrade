...
from sqlalchemy import BigInteger, Boolean, Column, DateTime, Float, Index, String, Text
...
class HistoricalNews(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "sentiment_magnitude IS NULL OR (sentiment_magnitude >= 0 AND sentiment_magnitude <= 1)",
            name="ck_news_magnitude_range",
        ),
        CheckConstraint(
            "market_relevance_score IS NULL OR (market_relevance_score >= 0 AND market_relevance_score <= 1)",
        ...
        # Content validation
        CheckConstraint(
            "headline IS NOT NULL AND trim(headline) != ''", name="ck_news_headline_not_empty"
        ),
