...
class DatabaseMetrics:
    ...
    def __init__(self, retention_minutes: int = 60):
        ...
        self.session_durations = deque(maxlen=1000)
    ...
    def record_session_closed(self, duration: float) -> None:
        ...
        if self.session_durations:
            self.connection_metrics.average_session_duration = sum(self.session_durations) / len(self.session_durations)
    ...
    def _update_baseline(self, query_type: str, execution_time: float) -> None:
        ...
        if query_type not in self.baseline_query_times:
            ...
            self.baseline_query_times[query_type] = alpha * execution_time + (1 - alpha) * current_baseline
    ...
    def get_performance_summary(self) -> dict[str, Any]:
        ...
        recent_queries = [q for q in self.query_history if datetime.now() - q.timestamp < timedelta(minutes=5)]
        ...
        recent_health_checks = [h for h in self.health_checks if datetime.now() - h['timestamp'] < timedelta(minutes=5)]
        health_success_rate = sum(1 for h in recent_health_checks if h['success']) / max(len(recent_health_checks), 1)
    ...
    def get_slow_queries_report(self) -> list[dict[str, Any]]:
        ...
        slow_query_summary = defaultdict(lambda: {
            'count': 0,
            'total_time': 0,
            'max_time': 0,
            'avg_time': 0,
        ...
        for query in self.slow_queries:
            ...
            slow_query_summary[query_type]['max_time'] = max(
                slow_query_summary[query_type]['max_time'], 
        ...
        return dict(slow_query_summary)
